...
Issue time t0 = last completed UTC hour at run time (ISSUE_STEP_H = 1; --t0 overrides), so a
delayed GitHub cron run still issues the freshest possible forecast with no observed/forecast overlap.
...
Outputs (data/sw72/):
  ...
  forecast_archive.json  past forecasts, one per >= ARCHIVE_STEP_H hours (last 21 days)
...
ISSUE_STEP_H = 1           # t0 grid in hours: 1 = last completed hour (cron cadence is set in the workflow)
ARCHIVE_STEP_H = 6         # archive an issue only if the newest archived one is >= this many hours older
PREVIOUS_MIN_AGE_H = 24    # "previous forecast" overlay = newest archived issue at least this old
...
def update_archive(entry, t0):
    """Add this issue if the newest archived issue is >= ARCHIVE_STEP_H older (keeps the file
    changing at most ~4x/day even when runs are delayed to odd hours); prune old entries."""
    arch = [a for a in load_archive() if a["t0"] != entry["t0"]]
    newest = max((pd.Timestamp(a["t0"]).tz_convert(None) for a in arch), default=None)
    if newest is None or t0 - newest >= pd.Timedelta(hours=ARCHIVE_STEP_H):
        arch.append(entry)
...
def run(t0):
    ...
    # --- outputs ---
    hist_idx = pd.date_range(t0 - pd.Timedelta(days=HISTORY_DAYS), min(raw.index[-1], t0), freq="h")
    ...
    previous = [a for a in archive if pd.Timestamp(a["t0"]).tz_convert(None) <= t0 - pd.Timedelta(hours=PREVIOUS_MIN_AGE_H)]
    prev_forecast = None
    if previous:
        a = previous[-1]
